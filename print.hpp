...
namespace nvparsehtml
{
    ...
    const bool print_no_indenting = false;   //!< Printer flag instructing the printer to suppress indenting of XML. See print() function.
    ...
    namespace internal
    {
        ...
        inline OutIt print_attributes(OutIt out, Node<Ch> *node)
        {
            ...
            {
                if (it->first.empty() || it->first == id) continue;
        ...
        inline OutIt print_children(OutIt out, Node<Ch> *node, int indent)
        {
            if (node->name() == String<Ch>("html", 4)) indent = 0;
