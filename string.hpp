...
namespace nvparsehtml
{
    ...
    inline bool operator< (const String<Ch>& lhs, const String<Ch>& rhs) {
        ...
        for (size_t i = 0; i < minLen; ++i ) {
            if( lhs_c[i] < rhs_c[i] ) return true;
            if( lhs_c[i] > rhs_c[i] ) return false;
        }
        if( lhs_length >= rhs_length ) return false;
        return true;
    }
    template<typename Ch>
    inline bool operator> (const String<Ch>& lhs, const String<Ch>& rhs) { return rhs < lhs; }
    template<typename Ch>
    inline bool operator<=(const String<Ch>& lhs, const String<Ch>& rhs) { return !(lhs > rhs); }
    template<typename Ch>
    inline bool operator>=(const String<Ch>& lhs, const String<Ch>& rhs) { return !(lhs < rhs); }
    ...
    inline bool compare_ci(const String<Ch>& lhs, const String<Ch>& rhs) {
        ...
        if (lhs_length != rhs_length) return false;
        for (size_t i = 0; i < lhs_length; ++i) {
            if (std::towlower(lhs_c[i]) != std::towlower(rhs_c[i])) return false;
        }
        return true;
    }
}
